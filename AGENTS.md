...
- `FreecamInteraction` 管理客户端/服务端模式及范围，替换模式内距离、放置边缘与输入入口；保留原版计时、权限和事件系统。联机未协商支持时仅观察。
- `FreecamSelection` 使用渲染矩阵选取，按实际形状外露面渲染；中键旋转、界面、失焦后需松开交互键再按下。Shift 保留辅助使用，不允许实体操作。
