...
- 文档布局（2026-09-14）：验收记录、实施计划与历史交接文档统一位于 `docs/`，索引为 `docs/README.md`。根目录仅保留项目入口与维护约定类 Markdown 文件。

- 蓝图缺料与任务存档（2026-09-12）：用户已确认补入普通 ME 终端后完工。当前施工步骤缺料时显示物品名称与无法取得的数量，ME 模拟不足回传首个实际缺口，保留身份/来源诊断与原匹配规则。线上任务管理器现接入已有 `.task` 存储：创建立即保存，世界保存/退出写盘，重进恢复为待施工，续建复核已建部分而不重复扣料；完成/取消写入终止记录，切换世界清空任务/会话/蓝图库缓存。旧版未落盘的已丢失任务无法恢复。Forge 冒烟与 lwjgl3ify 准备冒烟通过，新版已同步实例，提示效果与实际重进续建仍待实机确认。
