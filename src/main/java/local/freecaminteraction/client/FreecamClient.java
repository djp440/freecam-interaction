...
public final class FreecamClient {
    ...
    private static FreecamSession session;

    private FreecamClient() {}

    @EventBusSubscriber(modid = FreecamInteractionMod.MOD_ID, value = Dist.CLIENT, bus = EventBusSubscriber.Bus.MOD)
    public static final class Registration {
        private Registration() {}

        @SubscribeEvent
        public static void registerKeys(RegisterKeyMappingsEvent event) {
            enterKey = new KeyMapping("key.freecam_interaction.enter", KeyConflictContext.IN_GAME,
                    InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_G, "key.categories.freecam_interaction");
    ...
    public static boolean canBuild() {
        return session != null && session.canControl() && !session.dragging && !session.overClose()
                && FreecamInteraction.active(Minecraft.getInstance().player);
    }
