...
final class FreecamSession {
    ...
    float pitch = FreecamClient.PITCH;
    boolean dragging;
    private final boolean supported = client.getConnection().hasChannel(FreecamInteraction.Mode.TYPE);
    ...
    FreecamSession() {
        ...
        FreecamClient.clearMovement(originPlayer.input);
        ...
        if (supported) {
            FreecamInteraction.setActive(originPlayer, true);
            PacketDistributor.sendToServer(new FreecamInteraction.Mode(true));
        } else {
            originPlayer.displayClientMessage(Component.translatable("screen.freecam_interaction.unsupported"), false);
        }
        FreecamSelection.reset();
        ModLog.LOGGER.info("Freecam entered; dimension={}; anchor={}; previousCamera={}",
    ...
    void drag(boolean value) {
        if (dragging != value) {
            ModLog.LOGGER.info("Freecam drag={}; anchor={}; yaw={}; pitch={}", value, anchor, yaw, pitch);
        }
        dragging = value;
    ...
    void render(GuiGraphics graphics, KeyMapping toggle) {
        ...
        Component help = Component.translatable("screen.freecam_interaction.help",
        ...
        graphics.drawString(client.font, Component.translatable("screen.freecam_interaction.title"), 8, 8, 0xFFE6F8F5);
        ...
        if (overClose()) {
            graphics.renderTooltip(client.font, Component.translatable("screen.freecam_interaction.close"), width - 30, 32);
        }
    }
    ...
    void restore(String reason) {
        FreecamSelection.stopInput();
        FreecamSelection.reset();
        FreecamInteraction.setActive(originPlayer, false);
        if (supported && client.player == originPlayer && client.getConnection() != null
                && client.getConnection().hasChannel(FreecamInteraction.Mode.TYPE)) {
            PacketDistributor.sendToServer(new FreecamInteraction.Mode(false));
        }
        drag(false);
        KeyMapping.releaseAll();
        KeyMapping.resetToggleKeys();
        ...
        ModLog.LOGGER.info("Freecam exited; reason={}; restoredCamera={}; anchor={}; yaw={}; pitch={}",
