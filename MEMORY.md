...
- 2026-09-14 13:05：NeoForge 1.21.1 分支的验收记录与 Forge 历史交接文档移入 `docs/`，新增 `docs/README.md` 索引；交接文档内指向 Forge 计划的旧本机相对链接改为 GitHub `main` 分支链接。

- 2026-09-14 12:48：开源发布定位统一为“自由视角交互”，GitHub 仓库名为 `freecam-interaction`，Forge 1.7.10 作为默认 `main` 和主要开发版，NeoForge 1.21.1 转为 `neoforge-1.21.1` 次要维护分支。项目采用 MIT License；1.21.1 为保持存档和协议兼容，仅统一对外名称，保留技术标识 `godview_build`。两版发布前冒烟均通过。
