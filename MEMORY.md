...
- 2026-09-12 20:09：继续排查用户“mod 蓝图部分完成，ME/背包有料仍缺料”。日志 `logs/freecam_interaction/2026-09-12 19-30-43.log` 显示任务 `61bdce98-879c-484c-bd2d-b2df2a0c3f3b` 卡在 `(712,5,-1492)` 的 `appliedenergistics2:item.ItemMultiPart:380`（普通 ME 终端）；实际 `AE2网络材料建造测试-mod版` 蓝图有 3 方块、8 部件且全部材料 `matchTag=null`，排除该蓝图的额外 NBT 限制。实例“新的世界”当前 region 中 `(718,4,-1499)` 驱动器的存储元件仅余木板/驱动器/GT机器/GT管道/输出总线/GT覆盖板，无普通 ME 终端，但可能是用户取进背包后的状态；level.dat 与玩家 dat 修改时间停留于 9 月 10 日，不能据旧背包判定此次缺料。根因仍未确认，不放宽匹配或改写蓝图。`MaterialTransaction` 新增失败时需求、背包缺口/候选子类型/数量/NBT、光标堆及来源日志；`Ae2MaterialReservation` 新增来源/权限、ME 候选、模拟不足和实际带电提取不足日志，并保留异常堆栈。`LegacyActionCheck.materialTransactionChecks` 经真实事务入口验证匹配子类型背包供料、实际 NBT 回滚及不同子类型缺料不扣料。Forge 冒烟通过（`logs/tools/2026-09-12 20-08-40.log`），lwjgl3ify 准备冒烟通过并更新实例 JAR（`logs/tools/2026-09-12 20-09-08.log`）；待用户重启复现以取得实时服务端库存证据。未启动游戏、未修改存档、未提交，保留既有 `.diagnose-tmp/`。
